...
func TestGeminiChatRequestPreservesCompatibleUpstreamFields(t *testing.T) {
	...
	require.NoError(t, common.Unmarshal(output["generationConfig"], &generationConfig))
...
func TestGeminiChatRequestKnownFieldsTakePrecedenceOverExtraFields(t *testing.T) {
	...
	request.GenerationConfig.TopP = common.GetPointer(0.75)
	...
	require.NoError(t, common.Unmarshal(output["contents"], &contents))
	require.Len(t, contents, 1)
	require.Len(t, contents[0].Parts, 1)
	assert.Equal(t, "修改后的内容", contents[0].Parts[0].Text)

	var systemInstruction GeminiChatContent
	require.NoError(t, common.Unmarshal(output["systemInstruction"], &systemInstruction))
